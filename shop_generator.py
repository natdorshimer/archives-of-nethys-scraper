...
from aon_util import load_items_by_category, AonItemJson, load_items_by_categories
...
def get_random_items_by_request(search_request: EquipmentSearchRequest | ItemWithRunesSearchRequest) -> list[ItemOutputData] | list[
    AonItemJson]:
...
def get_cost_in_cp(price: str) -> int:
    ...
    return sum([int(price_split[i].split(' ')[0].replace(',', '')) * mappings[price_split[i].split(' ')[1]] for i in range(len(price_split))])
...
class ItemWithRunes:
    ...
    def get_strength_modifier_str(self) -> str:
        return (get_item_strength_by_name(self.strength.name, self.item_type_data.strength_name) + ' ') if self.strength else ''
...
def generate_items_with_runes(search_request: ItemWithRunesSearchRequest, item_type_data: ItemTypeData) -> list[ItemOutputData]:
    ...
    item_property_runes = [ item for item in equipment if item.item_subcategory == f'{item_type_data.potency_name} Property Runes']
